...
Enemy.reset = function (type, resetRoster) {
  ...
  for (var t = 0; t < Enemy.types.length; t++) { var x = Math.min(Math.max(Player.x + CONFIG.ENEMY_START_DISTANCE + t * 45, CONFIG.CANVAS_W), Level.pixelWidth() - 80); Enemy.spawnType(Enemy.types[t], x, Math.max(0, Player.y - CONFIG.ENEMY_SPAWN_HEIGHT)); }
};
Enemy.spawnType = function (type, x, y) {
  ...
  else { var actualType = Enemy.isType(type, "drill") ? "drill" : type, count = actualType === "drone" ? CONFIG.DRONE_COUNT : 1; for (var k = 0; k < count; k++) Enemy.enemies.push(Enemy.makeEnemy(actualType, x + k * 70, y - k * 45, actualType === "drone" ? CONFIG.CUOBID_SIZE / 2 : CONFIG.EVIL_SPIKE_SIZE / 2, actualType === "drone" ? 25 : 45, actualType === "drone" ? 210 : 120)); }
};
...
Enemy.update = function () { if (!Enemy.types.length) return; for (var i = 0; i < Enemy.enemies.length; i++) { var e = Enemy.enemies[i]; if (e.deadTimer > 0) { e.deadTimer--; if (e.deadTimer === 0) { e.hp = e.maxHp; e.x = Math.min(Math.max(Player.x + CONFIG.ENEMY_RESPAWN_DISTANCE, e.radius), Level.pixelWidth() - e.radius); e.y = Math.max(0, Math.min(CONFIG.CANVAS_H - e.radius, Player.y - CONFIG.ENEMY_SPAWN_HEIGHT + Math.random() * 180 - 90)); e.spawnX = e.x; e.spawnY = e.y; e.vx = 0; e.vy = 0; e.state = "windup"; e.timer = 0; e.grounded = false; e.windup = CONFIG.ENEMY_WINDUP_FRAMES; } continue; } if (e.state === "windup") { e.windup--; if (e.windup > 0) continue; e.state = "approach"; } if (e.type === "cuboid") Enemy.updateCuboid(e); if (e.type === "drone") Enemy.updateDrone(e); if (e.type === "drill") Enemy.updateDrill(e); if (e.type === "greenBall4") Enemy.updateGreenBall(e); } if (Game.hasCurse("unknownDimension")) Enemy.tryCombineCuboids(); Enemy.updateBullets(); };
