/* =====================================================================
   config.js -- ALL THE NUMBERS.
   Change the values in the ENEMIES section to tune enemy difficulty.
   ===================================================================== */
var CONFIG = {
  TILE: 40,
  ROWS: 10,
  PIECE_COLS: 8,
  CANVAS_W: 800,
  CANVAS_H: 400,

  MOVE_SPEED: 0.2,
  JUMP_POWER: 15,
  GRAVITY: 0.5,
  MAX_FALL: 16,
  PLAYER_SIZE: 32,
  PLAYER_RADIUS: 16,

  LINE_WIDTH: 3,
  DOT_DISTANCE: 0.55,

  // --- adjustable enemy variables --------------------------------------
  ENEMY_START_DISTANCE: 280,
  ENEMY_SPAWN_HEIGHT: 100,
  CUOBID_SPEED: 1.05,
  CUOBID_DISTANCE_SPEED: 0.0025,
  CUOBID_MAX_SPEED: 8,
  CUOBID_GROUP_SIZE: 5,
  CUOBID_SIZE: 36,
  CUOBID_SPAWN_SPACING: 500,
  CUOBID_MERGE_DISTANCE: 72,
  DRONE_FOLLOW_RATE: 0.035,
  DRONE_FAST_FOLLOW_RATE: 0.075,
  DRONE_HEIGHT_ABOVE_PLAYER: 105,
  DRONE_BULLET_SPEED: 6.5,
  DRONE_BULLET_LIFE: 100,
  DRONE_SHOT_INTERVAL: 14,
  DRONE_SHOTS: 5,
  DRONE_FIRE_TIME: 45,
  DRONE_BURST_COOLDOWN: 85,
  DRONE_FIRE_MOVE_FACTOR: 0.35,
  EVIL_SPIKE_WARNING_TIME: 30,
  EVIL_SPIKE_DASH_SPEED: 14,
  EVIL_SPIKE_DASH_TIME: 54,
  EVIL_SPIKE_SIZE: 38,
  DRILL_TURN_RATE: 0.14,
  GREEN_BALL_COUNT: 4,
  GREEN_BALL_BOUNCE: 9,
  GREEN_BALL_SIZE: 30,
  GREEN_BALL_JUMP_INTERVAL: 90,
  GREEN_BALL_JUMP_SPEED: 10,

  START_LEVEL: 0
};
